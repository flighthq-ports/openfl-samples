// the render.<backend>.ts of ts/src/DisplayingABitmap.
//
// Registers exactly the pieces this sample draws with, and nothing else — the same explicit opt-in
// the TypeScript tree makes, for the same reason: what is never registered can be dropped.
package;
